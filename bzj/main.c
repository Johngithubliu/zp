...
void sys_con() _task_ SYS_CON  {
	...
	static unsigned char pre_x59=0,x59,ppre_x59=0;
	...
	while(1)
 	{
		...
		//y32 ----------------
			...
			if(t_y55on)t_y55on--;
			if(t_y55on==1){set_y(55);t_y55off=delay_t[56];		}
...
*/
			...
			//X59--------
			x59=X59;
			if((x59)&&pre_x59&&(!ppre_x59))
			{
				if((error==0||error>=100))error=59;stop();
				//delay_t[62]=0;
				if(X56)delay_t[62]|=0x01;
				if(get_y(48))delay_t[62]&=~0x01;
				...
				while(X59) os_wait2(K_TMO,200);
			}
			if(x59&&pre_x59)ppre_x59=1;
			if(!x59&&!pre_x59)ppre_x59=0;
			pre_x59=x59;
...
void open_bag()	_task_	OPEN_BAG
{
	...
	while(1)
	{
		...
		if(status==RUNING)
		{
			...
			if(!X8){clear_y(18);if((error==0)||(error>100))error=8;clear_y(54);stop();continue;}
...
void press_22() _task_ PRESS_22
{
	...
	{	
		...
		if(Y63==0)
		{
			...
			{
				clear_y(16);
				if(status==RUNING)if((error==0)||(error>100)){if(!X22){error=22;clear_y(54);}else error=2;}
				f_open=0;
				moveout();
				stop();
