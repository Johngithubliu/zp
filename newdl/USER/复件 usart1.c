...
//					if(!Modbus_byte_count)Modbus_Counter++;
					...
					if(Modbus_Address==1503) //1003:Modbus地址	 1003
					{
						Modbus_pulse.pulse[3]=Modbus_0x10_Value[0];
						Modbus_pulse.pulse[2]=Modbus_0x10_Value[1];
						Modbus_pulse.pulse[1]=Modbus_0x10_Value[2];
						Modbus_pulse.pulse[0]=Modbus_0x10_Value[3];
					}
...
//int Cal_full_Mig;//标定用户砝码重量
					...
					{
						Modbus_Cal_user_weight.user_weight[3]=Modbus_0x10_Value[0];
						Modbus_Cal_user_weight.user_weight[2]=Modbus_0x10_Value[1];
						Modbus_Cal_user_weight.user_weight[1]=Modbus_0x10_Value[2];
						Modbus_Cal_user_weight.user_weight[0]=Modbus_0x10_Value[3];
...
//int Cal_user_weight;//标定砝码重量
					...
					{
						Modbus_Cal_weight.add_weight[3]=Modbus_0x10_Value[0];
						Modbus_Cal_weight.add_weight[2]=Modbus_0x10_Value[1];
						Modbus_Cal_weight.add_weight[1]=Modbus_0x10_Value[2];
						Modbus_Cal_weight.add_weight[0]=Modbus_0x10_Value[3];
...
//int Range_Filter0;//参数滤波范围
					if(Modbus_Address==Range_Filter0_address)//参数滤波范围
					{
						...
						STMFLASH_Write(FF_Range_Filter0,(u16*)&Range_Filter0,4);
					}
 //int Range_Filter1;//参数滤波范围
					if(Modbus_Address==Range_Filter1_address)//参数滤波范围
					{
						...
						STMFLASH_Write(FF_Range_Filter1,(u16*)&Range_Filter1,4);
					}
 //int Range_Filter2;//参数滤波范围
					if(Modbus_Address==Range_Filter2_address)//参数滤波范围
					{
						...
						STMFLASH_Write(FF_Range_Filter2,(u16*)&Range_Filter2,4);
					}
 //int Range_Filter3;//参数滤波范围
					if(Modbus_Address==Range_Filter3_address)//参数滤波范围
					{
						...
						STMFLASH_Write(FF_Range_Filter3,(u16*)&Range_Filter3,4);
					}
 //int Range_Filter4;//参数滤波范围
					if(Modbus_Address==Range_Filter4_address)//参数滤波范围
					{
						...
						STMFLASH_Write(FF_Range_Filter4,(u16*)&Range_Filter4,4);
					}
					/*******************************/
//int Strong_Filter0;//参数滤波强度
					if(Modbus_Address==Strong_Filter0_address)//参数滤波强度
					{
						...
						Strong_Filter0<<=8;
						Strong_Filter0|=Modbus_0x10_Value[1];
 ...
 //int Strong_Filter1;//参数滤波强度
					if(Modbus_Address==Strong_Filter1_address)//参数滤波强度
					{
						...
						Strong_Filter1<<=8;
						Strong_Filter1|=Modbus_0x10_Value[1];
 ...
 //int Strong_Filter2;//参数滤波强度
					if(Modbus_Address==Strong_Filter2_address)//参数滤波强度
					{
						...
						Strong_Filter2<<=8;
						Strong_Filter2|=Modbus_0x10_Value[1];
 ...
 //int Strong_Filter3;//参数滤波强度
					if(Modbus_Address==Strong_Filter3_address)//参数滤波强度
					{
						...
						Strong_Filter3<<=8;
						Strong_Filter3|=Modbus_0x10_Value[1];
 ...
 //int Strong_Filter4;//参数滤波强度
					if(Modbus_Address==Strong_Filter4_address)//参数滤波强度
					{
						...
						Strong_Filter4<<=8;
						Strong_Filter4|=Modbus_0x10_Value[1];
...
//
//			while(USART_GetFlagStatus(USART3, USART_FLAG_TXE) == RESET);
//			USART_SendData(USART3,' ');	//因为 USART2_DE_lowx; 较快，ADM2483 收变发 较慢 最后这一位发不出
//
//			while(USART_GetFlagStatus(USART3, USART_FLAG_TXE) == RESET);
//
////			elimate_USART_DE[3]=4;
//			USART3_RE_lowx;
//			USART3_DE_lowx;
//		 }
	}
...
//6.杂项数据组
STMFLASH_Read (FF_Clear_merory_code,(u16*)&Clear_merory_code,4);
STMFLASH_Read (FF_Target_pulse,(u16*)&Target_pulse,4);
STMFLASH_Read (FF_ID_weight_unit,(u16*)&ID_weight_unit,4);
STMFLASH_Read (FF_Num_shift,(u16*)&Num_shift,4);
...
STMFLASH_Read (FF_image_Pre_feed,(u16*)&image_Pre_feed,4);//参数提前量
