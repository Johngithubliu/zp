...
extern int Weighting_T,Weighting_package,Weighting_even;//³ÆÖØ_¶ÖÎ»£¬´üÊı£¬Æ½¾ùÖµ
...
//	while(1);
    ...
    //·¢ËÍÈ¥Êı¾İÒÑ¾­×¼±¸ºÃ£¬°´ÏÂ°´¼ü¼´¿ªÊ¼´«Êä
/*waitting button*/
    while(!GPIO_ReadInputDataBit(GPIOA, GPIO_Pin_0));
    
    printf("Start DMA transmission!\r\n");
    
    //ÕâÀïÊÇ¿ªÊ¼DMA´«ÊäÇ°µÄÒ»Ğ©×¼±¸¹¤×÷£¬½«USART1Ä£¿éÉèÖÃ³ÉDMA·½Ê½¹¤×÷
    USART_DMACmd(USART1, USART_DMAReq_Tx, ENABLE);
    //¿ªÊ¼Ò»´ÎDMA´«Êä£¡
    DMA_Cmd(DMA1_Channel4, ENABLE);
    printf("\r\n ");
...
//      {
//
//	        memset(Expression_Output,0,2048); //³õÊ¼»¯¡£ÒÔ±ãÔÚÄ©Î²¼Ó½áÊø·û'0'
//			STMFLASH_Read(FLASH_SAVE_ADDR,(u16*)Expression_Output,600);// ±í´ïÊ½
//												   
//			STMFLASH_Read(ADDR_POINTER,(u16*)Addr_expression,100);//Ã¿¸ö±í´ïÊ½Æ«ÒÆµØÖ·
//
//			STMFLASH_Read(Totall_Express,(u16*)&count_of_output,1);//±í´ïÊ½ÊıÁ¿
//
//			STMFLASH_Read(Expression_Len,(u16*)Len_expression,100);//Ã¿¸ö±í´ïÊ½³¤¶È
//
//			memset(Weight_parameter,0,300);//³õÊ¼»¯¡£ÒÔ±ãÔÚÄ©Î²¼Ó½áÊø·û'0'
//			STMFLASH_Read(Weight_para,(u16*)Weight_parameter,200); //ÖØÁ¿²ÎÊı×Ö·û´®
//			Desolve_weight_para(Weight_parameter);
//
//			memset(Step_parameter,0,300);//³õÊ¼»¯¡£ÒÔ±ãÔÚÄ©Î²¼Ó½áÊø·û'0'
//			STMFLASH_Read(Step_para,(u16*)Step_parameter,200);//²½½ø²ÎÊı×Ö·û´®
...
//			STMFLASH_Read (FL_Weighting_package,(u16*)&Weighting_package,2);
////			Weighting_T/=1000;
////			STMFLASH_Read(Weight_para,(u16*)Flash_Test_Weight_para,200); //ÖØÁ¿²ÎÊı×Ö·û´®
////			Desolve_weight_para(Weight_parameter);
////
////			STMFLASH_Read(Step_para,(u16*)Flash_Test_Step_para,200);//²½½ø²ÎÊı×Ö·û´®
////			Desolve_step_para(Step_parameter);
//	  }
...
/*********************************************************/
////while(Cal_been_opened)//À¥ÂØÍ¨Ì©±ê¶¨
////{
////	verify_filter_ADCODE();
//
////	KLTT_Cal_operation();
...
//combinate_letter_digtal(OUQ,digtal_x);
//		if(!programm_setting||Cal_Setting_step>0)	 //Éè¶¨½çÃæ
//		{
...
//Delay(0x02FFFF);
//Delay(0x02FFFF);
//if(!Command_01)//Ö´ĞĞ½â±í´ïÊ½£¬Êä³ö½á¹û
//		{
//			 simulator_package();//Êä³ö
//			 Store_XRCT_Status();//±ßÑØ²É¼¯
//		}
...
//				    USART_DMACmd(USART1, USART_DMAReq_Rx, ENABLE);
//				    DMA_Cmd(DMA1_Channel5, ENABLE);
//				    while(DMA_GetFlagStatus(DMA1_FLAG_HT5) == RESET); //DMA1_FLAG_HT5 +Delay(0x02FFFF);¿ÉÒÔ½µµÍ¶ÔDMAµÄÍ¨Ñ¶×Ö½ÚÊıµÄ¾«×¼¶È
...
//			
//			if(char_size==99998)
//				{GPIO_ResetBits(GPIOE, GPIO_Pin_6);count_of_output=seq-1; //±í´ïÊ½µÄ¸öÊı
//				    /*************±£´æ±í´ïÊ½£¬µØÖ· ±í´ïÊ½¸öÊı**********************/
//			        Stm32_Clock_Init(9);	//ÏµÍ³Ê±ÖÓÉèÖÃ Flash_Test_Area
//					STMFLASH_Write(FLASH_SAVE_ADDR,(u16*)Expression_Output,600);// ±í´ïÊ½
//					STMFLASH_Read(FLASH_SAVE_ADDR,(u16*)Flash_Test_Area,600);
//														   
//					STMFLASH_Write(ADDR_POINTER,(u16*)Addr_expression,100);//Ã¿¸ö±í´ïÊ½Æ«ÒÆµØÖ·
//					STMFLASH_Read(ADDR_POINTER,(u16*)Flash_Test_ADDR_Area,100);
//
//					STMFLASH_Write(Totall_Express,(u16*)&count_of_output,1);//±í´ïÊ½ÊıÁ¿
//					STMFLASH_Read(Totall_Express,(u16*)&Flash_count_of_output,1);
//
//					STMFLASH_Write(Expression_Len,(u16*)Len_expression,100);//Ã¿¸ö±í´ïÊ½³¤¶È
//					STMFLASH_Read(Expression_Len,(u16*)Flash_Test_Expression_Len,100);
///****************************************************************************************************/
//					STMFLASH_Write(Weight_para,(u16*)Weight_parameter,200);//ÖØÁ¿²ÎÊı×Ö·û´®
//					STMFLASH_Read(Weight_para,(u16*)Flash_Test_Weight_para,200);
//
//					STMFLASH_Write(Step_para,(u16*)Step_parameter,200);//²½½ø²ÎÊı×Ö·û´®
...
//    DMA_Cmd(DMA1_Channel4, ENABLE);
//	while(DMA_GetFlagStatus(DMA1_FLAG_TC4) == RESET);
//	Sum_Len_expression+=char_size;//¿éµÄ³¤¶È
//	seq++;
//	Addr_expression[seq]=Sum_Len_expression;//¿éµÄµØÖ·
//}
}    
#endif
